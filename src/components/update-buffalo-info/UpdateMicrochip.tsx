...
interface UpdateMicrochipProps {
  metadata: IMetadata;
}
...
const UpdateMicrochipDialog = ({ metadata }: UpdateMicrochipProps) => {
  ...
  const onSubmit = handleSubmit(async (data, event) => {
    ...
    const jsonMetadata = jsonMetadataGenerate(
      { ...metadata, microchip: data.newMicrochip },
      // metadata.image!,
  ...
  return (
    <dialog id="upload_microchip_dialog" className="modal">
      <div className="modal-box">
        <h3 className="text-lg font-bold">แก้ไข microchip</h3>
        <form onSubmit={onSubmit} className="grid grid-cols-1 gap-2">
          <input
            type="text"
            className="input"
            {...register("newMicrochip", { required: true })}
          ></input>
...
export default UpdateMicrochipDialog;
